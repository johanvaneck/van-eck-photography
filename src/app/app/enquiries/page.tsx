...
export default async function EnquiriesPage({ params }: { params: Promise<{ tenant: string }> }) {
    ...
    const { data: enquiries, error: errorEnquiries } = await getEnquiries(userId);
    if (errorEnquiries) {
        console.error("Error loading enquiries:", errorEnquiries);
        return <div>Error loading enquiries: {errorEnquiries.message}</div>;
    }
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className="text-left">Date of Interest</TableHead>
                        ...
                        <TableBody>
                            {enquiries.map((enquiry) => (
                                <TableRow key={enquiry.id} className="min-h-16">
                                    ...
                                    <TableCell className="text-left">{categories.find(cat => cat.id === enquiry.categoryId)?.name ?? ""}</TableCell>
                                    <TableCell className="text-left">{enquiry.dateOfInterest}</TableCell>
